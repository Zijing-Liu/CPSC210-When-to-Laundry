...
public class BookingApp {
    ...
    public BookingApp() throws FileNotFoundException {
        ...
        nameInput = "";
        timeSlotChosen = -1;
        bookingRecord = new BookingRecord("admin's booking record");
        jsonWriter = new JsonWriter(JSON_STORE);
        jsonReader = new JsonReader(JSON_STORE);
        thingy = new Thingy(null, -1);
        users = new ArrayList<>();
...
//            userBookings.add(currentUser);
    ...
    public void printBookingsList() {
        for (int i = 0; i < booking.getBookings().size(); i++) {
            ...
            if (!(booking.getBookings().get(i) == null)) {
    ...
    private void addThingy() {
        thingy = new Thingy(currentUser.geUserName(), currentUser.getBookedTime());
...
//        thingy.setBookedTime(currentUser.getBookedTime());
    ...
    private void printThingies() {
        List<Thingy> thingies = bookingRecord.getThingies();
    ...
    private void loadBookingRecord() {
        try {
